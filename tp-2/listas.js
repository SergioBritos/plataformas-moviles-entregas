...
// Ejemplos de uso
const resultado1 = crearListaDeNumeros(1, 5);
console.log(resultado1);  // Esto mostrará [1, 2, 3, 4, 5] en la consola

const resultado2 = crearListaDeNumeros(3, 6);
console.log(resultado2);  // Esto mostrará [3, 4, 5, 6] en la consola

const resultado3 = crearListaDeNumeros(2, 2);
console.log(resultado3);  // Esto mostrará [2] en la consola
...
function encontrarPalabraMasCorta(listaDePalabras) {
  if (listaDePalabras.length === 0) {
    return null; // Manejo de lista vacía
  }

  let palabraMasCorta = listaDePalabras[0]; // Asumimos que la primera palabra es la más corta
...
function filtrarSoloPositivos(listaDeNumeros) {
  const numerosPositivos = listaDeNumeros.filter(numero => numero > 0);
...
function contarAprobados(listaDeNotas) {
  const notaMinimaAprobacion = 7;
  const notasAprobadas = listaDeNotas.filter(nota => nota >= notaMinimaAprobacion);
...
function filtrarSoloTruthy(listaDeValores) {
  const valoresTruthy = listaDeValores.filter(valor => Boolean(valor));
